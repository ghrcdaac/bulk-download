...
function inGranulesPage() {
    const baseUrl = "https://search.earthdata.nasa.gov/search/granules?p=";
    const projectIdUrl = "https://search.earthdata.nasa.gov/search/granules?projectId="
...
function updateDownloadButton() {
    ...
    return( 
        fetch(getCmrQueryLink(url, false))
            .then(res => res.json())
            .then((out) => {
...
function getCmrQueryLink(url, withFilters = true) {
    ...
    let link = (baseUrl + filter['concept_id'] + filter['polygon'] + filter['rectangle'] + filter['point'] + filter['temporal'] + `&downloadable=true&page_size=${page_size}&page_num=`);
...
chrome.runtime.onMessage.addListener(
    function(response, sender, sendResponse) {
        ...
        if (response.message && response.message == "clear-swal-fire") {
            ...
            // if(!response.loggedin){
            //     const loginWindow = window.open('https://urs.earthdata.nasa.gov/', loginWindow, 'width=600,height=600');
